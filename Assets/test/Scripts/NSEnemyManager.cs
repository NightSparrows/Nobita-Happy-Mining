...
public class NSnemyManager : MonoBehaviour
{
    [System.Serializable]
	...
	public void init(in string levelDataJsonText)
    {
        ...
        foreach (EnemyLevelData enemyLevelData in this.currentLevelData.enemySpawnInfos)
        {
            ...
            switch (enemyLevelData.mobType)
            {
                case 0:
                    // for test enemy
                    enemySpawnData.mobPrefab = Resources.Load<GameObject>("test/Prefabs/Enemy/TestEnemyPrefab");
    ...
    void Update()
    {
        ...
        for (int i = currentEnemyList.Count - 1; i >= 0; i--)
        {
			...
			if (health.isDead() && enemyController.deathAnimationOver())
			{
            ...
            for (int i = 0; i < enemySpawnData.enemyLevelData.spawnCount; i++)
            {
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        float length = (new Vector3(x * TileController.Size, 0, z * TileController.Size) - this.player.transform.position).magnitude;
                ...
                GameObject enemyGameObject = Instantiate(enemySpawnData.mobPrefab, new Vector3(x * TileController.Size, 0, z * TileController.Size), Quaternion.identity/* or random rotation? */);
                IEnemyController enemyController = enemyGameObject.GetComponent<IEnemyController>();
